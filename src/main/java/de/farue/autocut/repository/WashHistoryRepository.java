...
public interface WashHistoryRepository extends JpaRepository<WashHistory, Long> {
    ...
    Optional<WashHistory> findFirstByReservationTenantIsInAndStatusAndReservationDateIsBetweenOrderByReservationDateAsc(
        Set<Tenant> tenants, WashHistoryStatus status, Instant from, Instant until);
    ...
    List<WashHistory> findAllByStatusAndReservationDateIsBefore(WashHistoryStatus status,
        Instant time);

    List<WashHistory> findAllByMachineAndStatusAndReservationDateIsNotNull(LaundryMachine machine,
