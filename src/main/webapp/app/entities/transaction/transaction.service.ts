...
export class TransactionService {
  ...
  protected convertDateFromClient(transaction: ITransaction): ITransaction {
    const copy: ITransaction = Object.assign({}, transaction, {
      bookingDate: transaction.bookingDate != null && transaction.bookingDate.isValid() ? transaction.bookingDate.toJSON() : null,
      valueDate: transaction.valueDate != null && transaction.valueDate.isValid() ? transaction.valueDate.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((transaction: ITransaction) => {
        transaction.bookingDate = transaction.bookingDate != null ? moment(transaction.bookingDate) : null;
        transaction.valueDate = transaction.valueDate != null ? moment(transaction.valueDate) : null;
      });
    }
    return res;
  }
