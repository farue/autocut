...
export class InternalTransactionComponent implements OnInit {
  ...
  protected onSuccess(data: IInternalTransaction[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/internal-transaction'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.internalTransactions = data ?? [];
